...
def first_approx_tool(SP, t, x, y, z):
        ...
        ### AAAAAAA
        diff_t=np.linalg.norm((SP_t-SP_og)/lattice_space)
        diff_x=np.linalg.norm((SP_x-SP_og)/lattice_space)
        diff_y=np.linalg.norm((SP_y-SP_og)/lattice_space)
        diff_z=np.linalg.norm((SP_z-SP_og)/lattice_space)     
...
def inv_Jack(SP, t, x, y, z):
    jack=np.zeros((4, 4))
    # SP_og=SP[t, x, y, z, :]
    # for i in range(4):      ### Specifies which element of the delta to take
    #     res=SP[t+1, x, y, z, i]-SP[t, x, y, z, i]
    #     jack[0][i]=res
    # for i in range(4):      ### Specifies which element of the delta to take
    #     res=SP[t, x+1, y, z, i]-SP[t, x, y, z, i]
    #     jack[1][i]=res
    # for i in range(4):      ### Specifies which element of the delta to take
    #     res=SP[t, x, y+1, z, i]-SP[t, x, y, z, i]
    #     jack[2][i]=res
    # for i in range(4):      ### Specifies which element of the delta to take
    #     SP_og=SP[t, x, y, z, i]
    SP_og=SP[t, x, y, z, :]
    ...
    SP_y=SP[t, x, y+1, z, :]
    SP_z=SP[t, x, y, z+1, :]
    jack[0][:]=(SP_t-SP_og)/lattice_space
    jack[1][:]=(SP_x-SP_og)/lattice_space
    jack[2][:]=(SP_y-SP_og)/lattice_space
    jack[3][:]=(SP_z-SP_og)/lattice_space
    return jack
